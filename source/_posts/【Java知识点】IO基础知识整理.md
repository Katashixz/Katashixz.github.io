...
- 案例
  ...
  - InputStreamReader 和 OutputStreamWriter 就是两个适配器(Adapter)， 同时，它们两个也是字节流和字符流之间的桥梁。InputStreamReader 使用 **StreamDecoder(流解码器)**对字节进行解码，实现字节流到字符流的转换， OutputStreamWriter使用**StreamEncoder(流编码器)**对字符进行编码，实现字符流到字节流的转换。
- 与装饰器模式的区别
  - 装饰器模式
...
## 观察者模式
主打一个监听，定义对象间的一种一对多的依赖关系，当一个对象的状态发生改变时，所有依赖于它的对象都得到通知并被自动更新。
